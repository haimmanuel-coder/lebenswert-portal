...
export function PasswortwechselPflichtModal() {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="passwortwechsel-titel" style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(15, 23, 42, 0.66)", display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "max(16px, env(safe-area-inset-top)) 16px max(16px, env(safe-area-inset-bottom))" }}>
      <form onSubmit={(event) => { event.preventDefault(); if (istGueltig) wechsel.mutate({ altesPasswort, neuesPasswort }); }} style={{ width: "min(100%, 440px)", maxHeight: "calc(100dvh - 32px)", overflowY: "auto", overscrollBehavior: "contain", WebkitOverflowScrolling: "touch", margin: "auto 0", background: "#fff", borderRadius: 18, padding: 26, boxShadow: "0 20px 60px rgba(0,0,0,.35)" }}>
        ...
        <label htmlFor="startpasswort" style={labelStyle}>Startpasswort</label>
        <PasswordInput id="startpasswort" name="startpasswort" data-testid="passwortwechsel-startpasswort" autoFocus autoComplete="current-password" value={altesPasswort} onChange={(event) => setAltesPasswort(event.target.value)} style={inputStyle} />
        <label htmlFor="neues-passwort" style={labelStyle}>Neues persönliches Passwort</label>
        <PasswordInput id="neues-passwort" name="neues-passwort" data-testid="passwortwechsel-neues-passwort" autoComplete="new-password" value={neuesPasswort} onChange={(event) => setNeuesPasswort(event.target.value)} style={inputStyle} />
        ...
        <label htmlFor="neues-passwort-wiederholen" style={labelStyle}>Neues Passwort wiederholen</label>
        <PasswordInput id="neues-passwort-wiederholen" name="neues-passwort-wiederholen" data-testid="passwortwechsel-wiederholung" autoComplete="new-password" value={wiederholung} onChange={(event) => setWiederholung(event.target.value)} style={{ ...inputStyle, borderColor: wiederholung && wiederholung !== neuesPasswort ? "#dc2626" : "#d1d5db" }} />
